...
function App() {
  ...
  function normalizeGuest(value) {
    return {
      ...
      problemaSaude: value?.problemaSaude === undefined || value?.problemaSaude === null ? "" : normalizeBoolean(value.problemaSaude, ""),
      alergia: value?.alergia === undefined || value?.alergia === null ? "" : normalizeBoolean(value.alergia, ""),
      medicamentoControlado: value?.medicamentoControlado === undefined || value?.medicamentoControlado === null ? "" : normalizeBoolean(value.medicamentoControlado, ""),
      usaSpa: value?.usaSpa === undefined || value?.usaSpa === null ? "" : normalizeBoolean(value.usaSpa, ""),
      outraAlergia: value?.outraAlergia === undefined || value?.outraAlergia === null ? "" : normalizeBoolean(value.outraAlergia, ""),
  ...
  function validateGuest() {
    ...
    if (!String(guest.motivoEntrada || "").trim()) {
      errors.motivoEntrada = "Motivo da entrada é obrigatório.";
    }

    if (!String(guest.dataNascimento || "").trim()) {
      errors.dataNascimento = "Data de nascimento é obrigatória.";
    }

    if (!String(guest.cpf || "").trim()) {
    ...
    const healthFields = [
      { radio: "problemaSaude", detail: "problemaSaudeQual", label: "problema de saúde" },
      { radio: "alergia", detail: "alergiaQual", label: "alergia" },
      { radio: "medicamentoControlado", detail: "medicamentoQual", label: "uso de medicação controlada" },
      { radio: "usaSpa", detail: "usaSpaQual", label: "uso de substância psicoativa" },
      { radio: "outraAlergia", detail: "outraAlergiaQual", label: "outro cuidado/alergia" },
    ];

    for (const { radio, detail, label } of healthFields) {
      if (guest[radio] === null || guest[radio] === undefined || guest[radio] === "") {
        errors[radio] = `Informe se há ${label}.`;
      } else if (guest[radio] === true && !String(guest[detail] || "").trim()) {
        errors[detail] = "Campo obrigatório quando a resposta for Sim.";
      }
    }

    if (!guest.aceitouTermo) {
      errors.aceitouTermo = "É necessário confirmar a ciência das informações.";
    }
    ...
    if (Object.keys(errors).length > 0) {
      ...
      if (errors.dataAcolhimento || errors.motivoEntrada) {
        setCurrentSection("inicio");
      } else if (errors.nome || errors.dataNascimento) {
        setCurrentSection("pessoais");
      } else if (errors.cpf) {
        setCurrentSection("documentos");
      } else if (["problemaSaude","alergia","medicamentoControlado","usaSpa","outraAlergia","problemaSaudeQual","alergiaQual","medicamentoQual","usaSpaQual","outraAlergiaQual"].some(k => errors[k])) {
        setCurrentSection("beneficios");
      } else if (errors.aceitouTermo) {
        setCurrentSection("termo");
      }
      return false;
    }
  ...
  function renderSection() {
    switch (currentSection) {
      case "inicio":
        return (
          <>
            <SectionHeader eyebrow="Ficha de acolhimento" title="Informações iniciais" hasRequired />
            <div className="form-grid">
              <Field label="Data de acolhimento *" error={fieldErrors.dataAcolhimento}>
                <input type="date" name="dataAcolhimento" value={guest.dataAcolhimento} onChange={updateField} required />
              </Field>
              ...
              <RadioGroup
                ...
                falseValue="nao"
              />
              <Field label="Motivo da entrada *" full error={fieldErrors.motivoEntrada}>
                <textarea name="motivoEntrada" rows="4" value={guest.motivoEntrada} onChange={updateField} required />
              </Field>
            </div>
          </>
        );
      case "pessoais":
        return (
          <>
            <SectionHeader eyebrow="Identificação" title="Dados pessoais" hasRequired />
            <div className="form-grid">
              <Field label="Nome completo *" full error={fieldErrors.nome}>
                <input name="nome" value={guest.nome} onChange={updateField} required />
              </Field>
              <Field label="Data de nascimento *" error={fieldErrors.dataNascimento}>
                <input type="date" name="dataNascimento" value={guest.dataNascimento} onChange={updateField} required />
              </Field>
              <Field label="Idade (calculada automaticamente)">
                <input name="idade" value={guest.idade} readOnly tabIndex={-1} style={{ background: "var(--surface-muted, #f5f5f5)", cursor: "default" }} />
              </Field>
      ...
      case "documentos":
        return (
          <>
            <SectionHeader eyebrow="Documentação" title="Documentos apresentados" hasRequired />
            <div className="form-grid">
              <Field label="RG">
                <input name="rg" value={guest.rg} onChange={updateField} />
              </Field>
              <Field label="CPF *" error={fieldErrors.cpf}>
                <input name="cpf" value={guest.cpf} onChange={updateField} required />
              </Field>
      ...
      case "beneficios":
        return (
          <>
            <SectionHeader eyebrow="Benefícios e saúde" title="Condições informadas" hasRequired />
            ...
            <div className="form-grid">
              ...
              {renderHealthField("Algum tipo de alergia?", "alergia", "aleargiaQual")}
      ...
      case "termo":
        return (
          <>
            ...
            <label className={`acceptance ${fieldErrors.aceitouTermo ? "has-error" : ""}`}>
            ...
            {fieldErrors.aceitouTermo && <small className="field-error">{fieldErrors.aceitouTermo}</small>}
          </>
        );
      case "acompanhamento":
        return (
        ...
        return (
          <>
            ...
            <div className="review-grid">
              ...
              <ReviewItem label="Data de nascimento" value={formatDate(guest.dataNascimento)} />
              <ReviewItem label="Idade" value={guest.idade ? `${guest.idade} anos` : ""} />
  ...
  function renderHealthField(label, field, detailField) {
    return (
      <div className="health-row field-full">
        <fieldset className={`radio-group ${fieldErrors[field] ? "has-error" : ""}`}>
          <legend>{label} *</legend>
          <label>
            <input type="radio" name={field} value="true" checked={guest[field] === true} onChange={updateField} />
            Sim
          </label>
          <label>
            <input type="radio" name={field} value="false" checked={guest[field] === false} onChange={updateField} />
            Não
          </label>
          {fieldErrors[field] && <small className="field-error">{fieldErrors[field]}</small>}
        </fieldset>
        {guest[field] === true && (
          <Field label="Qual? *" error={fieldErrors[detailField]}>
            <input name={detailField} value={guest[detailField]} onChange={updateField} required />
          </Field>
        )}
      </div>
    );
...
function SectionHeader({ eyebrow, title, compact = false, hasRequired = false }) {
  return (
    <div className={`section-title ${compact ? "compact" : ""}`}>
      <span>{eyebrow}</span>
      <h2>{title}</h2>
      {hasRequired && (
        <p className="required-legend">Campos marcados com <strong>*</strong> são obrigatórios.</p>
      )}
    </div>
  );
}
